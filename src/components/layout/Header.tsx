        ...
        <div className="hidden lg:flex lg:gap-x-8">
          {navigation.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="text-sm font-medium leading-6 text-white hover:text-secondary transition-colors"
            >
              {item.name}
            </Link>
          ))}
          ...
          <div className="lg:hidden fixed inset-0 z-50">
            ...
            <div className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-primary px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-border">
              ...
              <div className="mt-6 flow-root">
                <div className="-my-6 divide-y divide-border/30">
                  <div className="space-y-2 py-6">
                    {navigation.map((item) => (
                      <Link
                        key={item.name}
                        href={item.href}
                        className="-mx-3 block rounded-lg px-3 py-2 text-base font-medium leading-7 text-white hover:bg-primary/80"
                        onClick={() => setMobileMenuOpen(false)}
                      >
                        {item.name}
                      </Link>
